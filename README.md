...
-----------------------------
Obviously, to start a Django project, you need to have Python installed.
This requires [Homebrew], which should already be installed.
We're also going to install [Pip], which is an easy way to install the Python packages we'll need later.
Finally, we're going to install [VirtualEnv], which creates an isolated Python environment for our Django project. This is important because we know that our code will work with a particular version of Python (the one we are currently using). It makes sure the code in our project doesn't break when we inevitably do machine-wide Python updates some time in the future.
...
This installs Venv:
  ...
  [1]: https://github.com/mxcl/homebrew/wiki/installation  "Homebrew"
  [2]: http://pypi.python.org/pypi/pip/                    "Pip"
...
That was a quick and dirty introduction to Git, but there's a lot more that it has to offer. There are many awesome Git tutorials online if you want to learn more.
...
You can also list users, vhosts, and permissions. See the [RabbitMQ Documentation] for more info.
...
[4]: http://www.rabbitmq.com/man/rabbitmqctl.1.man.html  "RabbitMQ Documentation"
